...
file_name = 'Boltzmann_results.xlsx'
...
'''
Hier können die Parameter ausgewählt werden für etwaige Experimente.
'''
...
def boltzmann_selection(population, beta):
    fitness_sum = sum(math.exp(calculate_fitness(chromosome) * beta * -1) for chromosome in population)
    selection_probabilities = [math.exp(calculate_fitness(chromosome) * beta * -1) / fitness_sum for chromosome in population]
    selected = random.choices(population, weights=selection_probabilities, k=len(population)-elite_size)
